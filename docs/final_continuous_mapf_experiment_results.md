...
Use the following prompt with a strong LLM to draft paper sections, tables, figures, or an ICRA-style narrative.
...
- Metrics: AtGoal is fraction of agents reaching goals; Coll is cumulative pair-timestep collision count; ArrPLR is path length ratio only over agents that arrived; PLR over all agents is less paper-friendly because failed agents can wander.
...
6. Suggested captions for three figures: method diagram, Set A ablation bar chart, and Set B/C generalization chart.

Tone: rigorous, precise, and not overclaiming safety. Avoid saying collision-free guarantee unless assumptions are explicitly stated, because empirical collision counts are nonzero.
```
