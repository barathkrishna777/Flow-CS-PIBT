...
- Ran **`scripts/compact_dataset.py`** on `data/preprocessed`: removed downsampled timestep files, re-encoded kept `.pt` to compact format (float16 / int32, map channel stripped per compaction design), ~**547k** files, ~**92 GB** on disk (order-of-magnitude shrink from pre-compaction).
...
- **Quick smoke:** `evals/rishi_quick_wave9_best.csv` (`scripts/eval_rishi_paper.py --quick`).
...
- **`scripts/train_full.py`** re-extracts zips and wipes/rebuilds `data/` for extraction — **do not** use it when relying on in-place `data/preprocessed` compaction; prefer **`python -m main_pys.train_flow`** with **`--preprocessed-dir data/preprocessed`**.
...
| Ablations: Euler steps, consensus, τ, model size, wait threshold (`scripts/sweep_inference.py`) | Not done |
...
- **Stub / pipeline:** `scripts/generate_continuous_data.py` (Phase 1.5 in plan) — ORCA (or similar) expert trajectories; store under e.g. `data/continuous/`.
...
- **Create** `scripts/eval_continuous.py`.
...
- **`scripts/generate_continuous_data.py`** may not exist yet; add when starting expert data for Phase 3.
...
# Full Rishi eval
python -m scripts.eval_rishi_paper -m checkpoints/large_scale_flow_wave9_compact_best.pt \
  -o evals/rishi_full_wave9_compact_best.csv

# Quick Rishi eval
python -m scripts.eval_rishi_paper -m <ckpt> --quick -o evals/quick.csv
```
