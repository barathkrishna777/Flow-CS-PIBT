% ==========================================================================
%  Experimental Results — Continuous-Space MAPF with Flow + CV-PIBT
...
Straight+CV-PIBT: goal-directed constant-velocity policy passed through our shield, isolating the shield contribution.
Flow+ORCA: learned flow velocities shielded by ORCA instead of CV-PIBT, isolating the learned-policy contribution.
...
Flow+CV-PIBT achieves 87.4\% AtGoal with 633~collisions,
...
Straight+CV-PIBT already reaches 77.4\% AtGoal,
...
Adding the learned flow prior on top of CV-PIBT yields a
...
Flow+CV-PIBT achieves 0.927 AtGoal at $N\!=\!50$ and 0.921 at $N\!=\!100$,
compared with 0.889/0.886 for Straight+CV-PIBT and
...
On near-distribution maps (Set~B), Flow+CV-PIBT maintains
its advantage: on \texttt{random-64-64-10} it reaches 61.4\% AtGoal
versus 50.5\% for Straight+CV-PIBT and 4.8\% for ORCA.
...
Straight+CV-PIBT (41.1\% at $N\!=\!50$, 39.4\% at $N\!=\!100$)
outperforms Flow+CV-PIBT (25.5\%, 21.7\%).
...
CV-PIBT must repeatedly override these proposals, incurring
backtracking overhead that a simple goal-directed policy avoids.
This result underscores two points:
(1)~CV-PIBT itself generalizes robustly to unseen environments,
...
seeds 123 and 456 still surpass Straight+CV-PIBT on
...
% ------------------------------------------------------------------
\subsection{CV-PIBT as the Primary Contribution}\label{sec:shield-contrib}

The experimental decomposition provides clear evidence that
CV-PIBT is the primary algorithmic contribution of this work.
ORCA and PO-ORCA both fail categorically on dense random maps,
achieving under 17\% completion at $N\!=\!100$ regardless of the
velocity source.
Replacing the shield with CV-PIBT while retaining a naive
...
(Set~B and warehouse maps in Set~C), where CV-PIBT delivers
...
over Straight+CV-PIBT on in-distribution and
...
layouts: on \texttt{warehouse} maps, Flow+CV-PIBT
underperforms Straight+CV-PIBT by 15.6~pp ($N\!=\!50$)
and 17.7~pp ($N\!=\!100$).
...
While all seeds outperform Straight+CV-PIBT, the margin
...
nonzero.
CV-PIBT does not provide a formal collision-avoidance
...
\caption{%
  ...
  Integration-step ablation uses Flow+CV-PIBT with $k$ Euler steps.
...
\midrule
\multicolumn{18}{l}{\textit{Integration-step ablation} (Flow+CV-PIBT, 512 steps)} \\
\cmidrule(lr){2-18}
& $k\!=\!3$ (default)
  & .989 &  20 & 1.24 &  82
  & .968 &  84 & 1.28 & 104
...
& $k\!=\!20$
  ...
  & .868 & 730 & 1.66 & 183 \\
\midrule
\multicolumn{18}{l}{\textit{Multi-seed robustness} (Flow+CV-PIBT, 512 steps, AtGoal / Coll)} \\
\cmidrule(lr){2-18}
& Seed 42
  & .984 & --- & --- & ---
  & .981 & --- & --- & ---
...
\multirow{3}{*}{\shortstack[l]{\texttt{random-64}\\$N\!=\!50$}}
  & ORCA             & .048 &  383 & 0.99 \\
  & Straight+CV-PIBT   & .505 &  267 & 1.05 \\
  & Flow+CV-PIBT       & \textbf{.614} &  106 & 1.39 \\
\midrule
\multirow{3}{*}{\shortstack[l]{\texttt{room-32}\\$N\!=\!50$}}
  & ORCA             & .012 & 1028 & --- \\
  & Straight+CV-PIBT   & .114 & 2020 & --- \\
  & Flow+CV-PIBT       & \textbf{.262} & 1113 & --- \\
\midrule
\multirow{3}{*}{\shortstack[l]{\texttt{warehouse}\\$N\!=\!50$}}
  & ORCA             & .132 &  855 & --- \\
  & Straight+CV-PIBT   & \textbf{.411} &   30 & --- \\
  & Flow+CV-PIBT       & .255 &   57 & --- \\
\midrule
\multirow{3}{*}{\shortstack[l]{\texttt{warehouse}\\$N\!=\!100$}}
  & ORCA             & .125 & 3496 & --- \\
  & Straight+CV-PIBT   & \textbf{.394} &  168 & --- \\
  & Flow+CV-PIBT       & .217 &  320 & --- \\
\bottomrule
\end{tabular}
\end{table}
