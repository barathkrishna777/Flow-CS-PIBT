...
Shielding~\cite{alshiekh2018shielding} is a technique from formal methods that wraps a learned policy with a verifiable safety filter: if the policy's action is unsafe, the shield substitutes a certified safe action. In our setting, the shield is not formally verified (geometric constraints admit counterexamples under concurrent motion), but CV-PIBT provides strong empirical safety by explicitly checking collision geometry for every committed velocity before it is executed.
